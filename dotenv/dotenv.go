...
func BytesFor[T any](data []byte, options Options) (config.Source, error) {
	...
	prepared.data = bytes.Clone(data)
	return prepared, nil
}
...
func FromFSFor[T any](filesystem fs.FS, path string, options Options) (config.Source, error) {
	...
	if filesystem == nil {
		return nil, errors.New("dotenv source filesystem must not be nil")
	}
	if !fs.ValidPath(path) {
		return nil, fmt.Errorf("dotenv source path %q is invalid", path)
	}
	prepared.open = func() (io.ReadCloser, error) {
...
func (s *source) Load(ctx context.Context) (config.Document, error) {
	...
	if s.open != nil {
		...
		data, err = readBounded(ctx, reader, s.limits.MaxBytes)
	...
	if int64(len(data)) > s.limits.MaxBytes {
		return config.Document{}, fmt.Errorf("dotenv input exceeds %d byte limit", s.limits.MaxBytes)
	}
...
func readBounded(ctx context.Context, reader io.Reader, maxBytes int64) ([]byte, error) {
	...
	if int64(buffer.Len()) > maxBytes {
		return nil, fmt.Errorf("dotenv input exceeds %d byte limit", maxBytes)
	}
