...
var defaultFileOperations = fileOperations{
	...
	lstat: os.Lstat, stat: os.Stat, relative: filepath.Rel,
...
func search(ctx context.Context, options Options, operations fileOperations) ([]Result, error) {
	...
	if configured.startAbs != "" {
		...
		for depth := 0; ; depth++ {
			...
			if !configured.Upward || samePath(directory, configured.stopAbs) {
				break
			}
...
func validateWith(options Options, operations fileOperations) (settings, error) {
	...
	for _, place := range options.SearchPlaces {
		if place == "" || filepath.Base(place) != place || place == "." || place == ".." {
			return settings{}, fmt.Errorf("configuration search place %q must be a filename", place)
		}
	}
	...
	if options.Upward && !configured.within(configured.stopAbs, configured.startAbs) {
...
func (s settings) absolute(path string) (string, error) {
	...
	if s.rootAbs != "" && !s.within(s.rootAbs, absolute) {
...
func (s settings) inspect(path string, candidate candidate) (Result, bool, error) {
	...
	expected := path
	if s.rootAbs != "" {
		relative, err := s.operations.relative(s.rootAbs, path)
		if err != nil {
			return Result{}, false, err
		}
		expected = filepath.Join(s.rootResolved, relative)
	}
	symlink := !samePath(expected, resolved)
	if symlink && s.Symlinks == RejectSymlinks {
		return Result{}, false, fmt.Errorf("%w: %s", ErrSymlink, path)
	}
	if s.rootResolved != "" && !s.within(s.rootResolved, resolved) {
		return Result{}, false, fmt.Errorf("%w: %s", ErrOutsideRoot, resolved)
	}
	...
	if s.Permissions == OwnerOnly && resolvedInfo.Mode().Perm()&0o077 != 0 {
		return Result{}, false, fmt.Errorf("%w: %s", ErrPermissions, path)
	}
	return Result{
...
func (s settings) within(parent, child string) bool {
	return withinWith(parent, child, s.operations.relative)
}
