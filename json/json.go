...
func Bytes(data []byte, options Options) (config.Source, error) {
	...
	return &source{info: info, data: bytes.Clone(data), limits: limits}, nil
}
...
func FromFS(filesystem fs.FS, path string, options Options) (config.Source, error) {
	...
	if filesystem == nil {
		return nil, errors.New("JSON source filesystem must not be nil")
	}
	if !fs.ValidPath(path) {
		return nil, fmt.Errorf("JSON source path %q is invalid", path)
	}
...
func (s *source) Load(ctx context.Context) (config.Document, error) {
	...
	if int64(len(data)) > s.limits.MaxBytes {
		return config.Document{}, fmt.Errorf(
			"JSON source %q exceeds %d byte limit",
			s.info.Name,
			s.limits.MaxBytes,
		)
	}
...
func readBounded(ctx context.Context, reader io.Reader, maxBytes int64) ([]byte, error) {
	...
	buffer := bytes.NewBuffer(make([]byte, 0, min(maxBytes, 32*1024)))
	...
	if int64(buffer.Len()) > maxBytes {
		return nil, fmt.Errorf("JSON input exceeds %d byte limit", maxBytes)
	}
