...
func TestLoaderSnapshotsOptionSlicesAndReadsProcessEnvironmentPerLoad(t *testing.T) {
	t.Setenv("APP_PORT", "1000")
	first, err := programmatic.Defaults("first", map[string]any{"port": int64(10)})
	if err != nil {
		t.Fatal(err)
	}
	second, err := programmatic.Defaults("second", map[string]any{"port": int64(20)})
	if err != nil {
		t.Fatal(err)
	}
	sources := []config.Source{first}
	validators := []validation.Validator[settings]{
		func(context.Context, settings) error { return nil },
	}
	loader, err := configservice.New(configservice.Options[settings]{
		Sources:    config.DefaultSources{Defaults: sources},
	...
	sources[0] = second
	validators[0] = func(context.Context, settings) error {
		return errors.New("late validator mutation")
	}
	loaded, err := loader(context.Background(), service.Invocation{})
	if err != nil {
		t.Fatalf("loader() error = %v", err)
	...
	if loaded.Port != 10 {
		t.Fatalf("Port = %d, want snapshotted source value 10", loaded.Port)
	}

	environmentLoader, err := configservice.New(configservice.Options[settings]{
		Environment: &environment.Options{Name: "environment", Prefix: "APP_"},
	})
	if err != nil {
		t.Fatalf("New(environment) error = %v", err)
	}
	t.Setenv("APP_PORT", "2000")
	loaded, err = environmentLoader(context.Background(), service.Invocation{})
	if err != nil {
		t.Fatalf("environment loader() error = %v", err)
	}
	if loaded.Port != 2000 {
		t.Fatalf("Port = %d, want current process environment value 2000", loaded.Port)
	}
}
