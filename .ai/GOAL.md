...
`go-config` owns configuration discovery, source loading, merging, decoding,
...
such as `go-postgres`, `go-telemetry`, `go-filesystem`, `go-queue`, and
`go-authentication` define the configuration types they own. `go-config` loads
...
- `go-config` MUST NOT impose a general validation framework on applications.
...
- `go-config` consumes the resulting environment variables or mounted files like
...
- `go-service` uses `go-config` for startup configuration and no longer owns
  environment parsing.
- `go-wire` may provide bounded JSON/YAML/TOML decoding internals where its API
  preserves the strict configuration semantics required here.
- `go-log` and `go-telemetry` integrations expose source and validation events
...
4. Complete secret redaction, hostile-input, race, cross-platform, and benchmark
   hardening.
5. Integrate `go-service`, package-owned config types, and Kubernetes recipes.
