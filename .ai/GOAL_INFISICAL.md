...
`go-config` without turning configuration core into a secret-management SDK or
...
Infisical is the system of record and rotation authority. `go-config` is a
...
- `go-config` performs an ordinary boot load and has no Infisical SDK dependency
  in this mode.
...
- The preferred packaging is a separately versioned nested adapter module with a
  documented compatibility matrix against `go-config` core.
...
- generic `go-config` snapshot publication and subscription contracts
...
- Infisical SDK dependency entering `go-config` core.
